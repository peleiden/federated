...
NAMES["alpha-df"] = r"FedDF: Class balance ($\alpha$)"
NAMES["noisy_clients"] = "Noisy clients ($N_K$)"
NAMES["noisy_clients-df"] = r"FedDF: Noisy clients ($N_K$)"
...
def table_over_arg():
    ...
    for var in NAMES.keys():
        ...
        values = np.array([cfg[var.replace("-df", "")] for cfg in configs])
        if "alpha" in var:
            r2, c2 = results_and_cfgs("iid" if "df" not in var else "iid-df")
            results.extend(r2)
            configs.extend(c2)
