...
Firstly, the results show that model convergence is possible even though data is federated across devices; a conclusion substantiated by the fact that running the $E=1$ experiment converged to in 95\pro\ of centralised learning performance as shown in Appendix Table \ref{app:centralised}.
...
If training time is limited, more local epochs might be optimal, as seen in minutes 5 to 15 using Ethernet, and all the way to minute 40 for Wi-Fi.
...
Even using only 5 clients each round, the final accuracy reached $93 \pro$ of using all 40, emphasizing the stability of the model averaging.
\textcolor{red}{
TODO: Add plot that shows that fewer clients sampled means slower convergence.  Probably in appendix.
}
