...
This was done by storing all $K$ client datasets on each device and for every communication round $l$, assigning Raspberry Pi's to train on one of the $S$ sampled client datasets.

The Pi's were connected to a switch connected via cable to the router.
When turning off the switch, the Pi's were set to instead connect via Wi-Fi, allowing tests of the impact of communication overhead in two cases: under relatively fast Ethernet and relatively slow Wi-Fi.
For reference, the network used had a bandwidth of 100/100 Mbit/s, all of which would be utilised on Ethernet, but only about 40\pro\ on Wi-Fi.
