...
The algorithm changes the ordering within each $\bm\pi_i$ iteratively, until $u$ converges.
\begin{algorithm}
...
\EndFor
\State $u_{\text{current}} \gets u(\mathbf P)$
\State $u_{\text{previous}} \gets \infty$
\While{$u_{\text{current}} < u_{\text{previous}}$}
    \State stuff
\EndWhile
\end{algorithmic}
\end{algorithm}
