...
A global model $\mathcal M_G$ was initialized and maintained on the central server, and for $L$ iterations, dubbed communication rounds, $S \leq K$ clients were sampled, each receiving a copy of the model.
...
The project setup, shown at Figure \ref{fig:setup}, was divided into two pieces: A central high-performance cluster (HPC) and 20 Raspberry Pi 3B's, each with 1GB memory and a quad-core 1.2GHz CPU.
...
Every Pi ran a Flask server that receiving a model every communication round and returning the trained local model along with telemetry data such as memory usage, which is an important consideration when running on such resource-limited devices.
...
This implementations stored the entire training dataset on each physical device, making it possible to simulate more than 20 clients ($K > 20$) as long as no more than 20 clients were sampled each round ($S \leq 20$).

Each Pi was connected to a switch, which in turn is connected via cable to the router.
If the switches were turned off, the Pi's connect to the router via Wi-Fi, which allows for testing the impact of communication overhead in two cases: under relatively fast ethernet and relatively slow Wi-Fi.
...
Due to device memory limits, all images were grayscaled.

The training dataset containing 50K images of 10 equally sized classes.
For the model $\mathcal M$, we chose a small convolutional neural network known to be able to achieving high performance on the simpler CV task, MNIST \cite{chollet2015keras}.
The network has 406,922 trainable parameters and is described in Appendix \ref{app:model}.
