...
A simple way to distribute data is to scramble and divide it into $K$ contiguous chunks, which would make the label distribution on each client approximately uniform.
Following the literature, we denote this distribution scheme iid. \cite{lin2020ensemble}

However, in practice, such balance in the dataset can rarely be assumed in FL setting \cite{kai2021advances}.
...
\begin{table}[htb!]
    ...
    \caption{Baseline parameters used for all experiments.
        The values were based on pilot experiments.}
    \label{tab:baseline}
\end{table}
...
\begin{figure}[htb!]
    ...
    \caption{
        TODO: CHANGE TO BETTER EXPERIMENT WITH 10 LOCAL EPOCHS (IF WE CAN AGREE)
        A training trajectory when recording running accuracies on each client in each communication round ehere each faint line correspond to $E=20$ local epochs performances of each of the $S=20$ sampled clients.
    }
    \label{fig:accuracy}
\end{figure}\noindent
